...
# It uses the Triple-Barreled Continuation Monad for resolution, backtracking,
# and branch pruning.
#
#
#   “The continuation that obeys only obvious stack semantics, O grasshopper,
#   is not the true continuation.” — Guy Steele.
#
#
...
# and 'unify_any' for unification. The resolution process is started by calling
# 'resolve' on a goal and then iterating over the solutions, which consist of
# substitution environments (proxy mappings) of variables to their bindings.
#
...
#
# Due to the absence of Tail Call Elimination in Python, Trampolining with
...
def tailcall(cont: Step | Emit | Next) -> Callable[..., Result]:
...
def unify_any(variable: Variable, *values) -> Goal:
    """Tries to unify a variable with any one of objects.
    Fails if no object is unifiable."""
    # pylint: disable=E1101
    # type: ignore
    return amb.from_iterable(unify((variable, value)) for value in values)
...
def resolve(goal: Goal) -> Iterable[Mapping]:
    ...
    while result:
        subst, backtrack = result  # pylint: disable   # type: ignore
        if subst:
            yield subst.proxy
        result = backtrack()
